...
class containerAbstract{
    ...
    Point _orgin;
    Point _divPoint;
    unordered_map<u_int32_t,Agent*> _lagh;
...
public:
    template<typename T = Point,
            ...
            containerAbstract(T &&offset,T &&ab,T &&orgin,
                    unordered_map<u_int32_t,Agent*> &&vecAgent,u_int bigAbID)
    :_offset(std::forward<T>(offset)),_abstract(std::forward<T>(ab)),
    _orgin(std::forward<T>(orgin)),_divPoint(_orgin/_abstract),_lagh(vecAgent),
    idxBigScope(bigAbID){
    }
    ~containerAbstract()
    {
    ...
    Agent* get_agent(const State* s)
    {
        ...
        if(auto pos = _lagh.find(entry); pos==_lagh.end())
            assert(false);
        else
            return pos->second;
    }
